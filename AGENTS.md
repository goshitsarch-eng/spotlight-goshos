...
an unredirected fullscreen window bypasses composition so even top chrome is invisible open() holds unredirect via Meta.Compositor.disable_unredirect on 48-50 or Meta.disable_unredirect_for_display on 45-47 close() hides the popup and backdrop first then releases that hold once so a fullscreen surface cannot scanout through leftover chrome disable/enable are a matched pair do not enable without a hold https://gitlab.gnome.org/GNOME/gnome-shell/-/blob/gnome-50/js/ui/boxpointer.js
...
7. the prefs combo must not apply on init see shouldApplyLook so a custom icon size survives reopening appearance reset look calls applyLookSettings for the current id because the combo will not
