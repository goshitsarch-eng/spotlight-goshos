...
assert(isUrlQuery('file://nas/Public Share'), 'network file uri with space');
assert(isFileUrlQuery('file://nas/Public Share'), 'network file helper accepts space');
assertEq(normalizeUrl('file://nas/Public Share'), 'file://nas/Public%20Share', 'encode network file uri');
assert(isUrlQuery('file://localhost/home/u/My Documents'), 'localhost file with space');
assertEq(normalizeUrl('file://localhost/home/u/My Documents'), 'file:///home/u/My%20Documents', 'localhost file encodes');
...
assert(isUrlQuery('sftp://nas/My Documents'), 'sftp uri with space');
assert(isRemoteLocationQuery('sftp://nas/My Documents'), 'remote helper accepts space');
assert(!isRemoteLocationQuery('sftp://'), 'bare sftp is not a location');
assertEq(normalizeUrl('sftp://nas/My Documents'), 'sftp://nas/My%20Documents', 'encode pasted sftp uri');
assertEq(normalizeUrl('smb://nas/Public Share'), 'smb://nas/Public%20Share', 'encode pasted smb uri');
...
assertEq(urlRowDescription('file://nas/Public%20Share'), 'Open location', 'network file copy');
...
assertEq(urlRowIcon('file://nas/share'), 'network-server-symbolic', 'network file icon');
...
assertEq(nextToggleAction(false, false, false, false), 'open', 'idle toggle opens');
assertEq(nextToggleAction(false, false, true, false), 'cancel-open', 'pending open cancels');
assertEq(nextToggleAction(true, false, false, false), 'close', 'open flag closes');
assertEq(nextToggleAction(false, true, false, false), 'close', 'visible closes');
assertEq(nextToggleAction(true, false, false, true), 'toggle-reopen', 'pending close reopens');
assertEq(nextToggleAction(false, false, true, true), 'toggle-reopen', 'close pending wins');
...
assertEq(nextScaleListenAction(true, {scale_factor: 2}), 'keep', 'existing scale listen is kept');
assertEq(nextScaleListenAction(false, null), 'wait', 'missing theme context waits');
assertEq(nextScaleListenAction(false, {scale_factor: 2}), 'listen', 'late theme context attaches');
...
assertEq(nextUnredirectAction(false, true, 'compositor'), 'hold', 'first open holds unredirect');
assertEq(nextUnredirectAction(true, true, 'compositor'), 'keep', 'already held does not stack');
assertEq(nextUnredirectAction(true, false, 'compositor'), 'release', 'close releases the hold');
assertEq(nextUnredirectAction(false, false, 'compositor'), 'keep', 'closed popup does not enable');
assertEq(nextUnredirectAction(false, true, ''), 'keep', 'missing api does not hold');
...
assertEq(appMatchTier('Files', '', 'org.gnome.Nautilus.desktop', [], 'nautilus'), 4, 'last id component');
assertEq(appMatchTier('Firefox', '', 'org.mozilla.firefox.desktop', [], 'org'), -1, 'org is not every desktop id');
assertEq(appMatchTier('Firefox', '', 'org.mozilla.firefox.desktop', [], 'zil'), -1, 'zil is not mozilla');
...
assertEq(appMatchTier('Notes', '', 'notes.desktop', [], 'ite', 'Write notes and lists'), -1, 'ite is not write');
assertEq(appMatchTier('Firefox', 'Web Browser', 'firefox.desktop', [], 'ows'), -1, 'ows is not browser');
...
assert(wordPrefixMatch('~/documents', 'doc'), 'slash boundary');
assert(!wordPrefixMatch('/home/u', 'ome'), 'ome is not a path segment');
assert(pathMatchesQuery('~/Documents', 'doc'), 'path segment prefix');
assert(!pathMatchesQuery('/home/u', 'ome'), 'path mid-string miss');
assert(!wordPrefixMatch('chrome', 'chro'), 'first word is startsWith not wordPrefix');
assert(idMatchesQuery('org.mozilla.firefox', 'mozilla'), 'dotted id word');
assert(idMatchesQuery('org.mozilla.firefox', 'fire'), 'last id component prefix');
assert(!idMatchesQuery('org.mozilla.firefox', 'org'), 'org is not an id match');
assert(!idMatchesQuery('org.mozilla.firefox', 'zil'), 'zil is not mozilla');
assert(idMatchesQuery('Firefox Navigator org.mozilla.firefox', 'nav'), 'class instance word');
assert(!idMatchesQuery('Firefox Navigator org.mozilla.firefox', 'org'), 'joined class keeps org out');
assert(!idMatchesQuery('org.mozilla.firefox', 'f'), 'short id prefix stays a name match');
assert(labelMatchesQuery('Web Browser', 'bro'), 'generic word prefix');
assert(!labelMatchesQuery('Web Browser', 'ows'), 'ows is not browser');
assert(labelMatchesQuery('Write notes and lists', 'write'), 'comment prefix');
assert(!labelMatchesQuery('Write notes and lists', 'ite'), 'ite is not write');
...
assert(!windowMatches('Notes', 'org.gnome.TextEditor', 'org'), 'org is not every window class');
assert(!windowMatches('Notes', 'org.gnome.TextEditor', 'ome'), 'ome is not gnome');
...
assert(!recentFileMatches('notes.txt', '/home/u', 'ome'), 'ome is not a recent folder');
...
assertEq(canonicalizeFileUri('file:///home/u/My Documents'), 'file:///home/u/My%20Documents', 'space file uri');
assertEq(canonicalizeFileUri('file:///home/u/My%20Documents'), 'file:///home/u/My%20Documents', 'encoded file uri stays');
assertEq(canonicalizeFileUri('file://nas/Public Share'), 'file://nas/Public%20Share', 'network file space');
assertEq(canonicalizeFileUri('file://nas/Public%20Share'), 'file://nas/Public%20Share', 'network file encoded stays');
assertEq(canonicalizeFileUri('FILE:///home/u/My Documents'), 'file:///home/u/My%20Documents', 'uppercase file scheme');
assertEq(canonicalizeFileUri('file://localhost/home/u/My Documents'), 'file:///home/u/My%20Documents', 'localhost file encodes');
assertEq(canonicalizeFileUri('sftp://host/x'), 'sftp://host/x', 'remote uri unchanged');
assertEq(canonicalizeRemoteUri('sftp://nas/My Documents'), 'sftp://nas/My%20Documents', 'sftp space');
assertEq(canonicalizeRemoteUri('sftp://nas/My%20Documents'), 'sftp://nas/My%20Documents', 'sftp encoded stays');
assertEq(canonicalizeRemoteUri('sftp://user@nas:22/My Documents'), 'sftp://user@nas:22/My%20Documents', 'sftp user port');
assertEq(canonicalizeRemoteUri('smb://nas/Public Share/file'), 'smb://nas/Public%20Share/file', 'smb nested space');
assertEq(canonicalizeLaunchUri('file:///home/u/My Documents'), 'file:///home/u/My%20Documents', 'launch file space');
assertEq(canonicalizeLaunchUri('file://nas/Public Share'), 'file://nas/Public%20Share', 'launch network file space');
assertEq(canonicalizeLaunchUri('sftp://nas/My Documents'), 'sftp://nas/My%20Documents', 'launch sftp space');
assertEq(canonicalizeLaunchUri('https://example.com/a b'), 'https://example.com/a b', 'https launch unchanged');
assertEq(pathFromFileUri('file:///home/u/My Documents'), '/home/u/My Documents', 'unencoded space path');
assertEq(pathFromFileUri('file://localhost/home/u/My Documents'), '/home/u/My Documents', 'localhost file path');
assertEq(pathFromFileUri('FILE:///home/u/x'), '/home/u/x', 'uppercase file path');
...
assert(!placeMatches('Documents', ['documents', 'docs'], 'ume'), 'ume is not documents');
...
assertEq(normalizeBookmarkUri('file:///home/u/My Documents'), 'file:///home/u/My%20Documents', 'bookmark file space');
assertEq(normalizeBookmarkUri('file://nas/Public Share'), 'file://nas/Public%20Share', 'bookmark network file space');
assertEq(normalizeBookmarkUri('FILE:///home/u/x'), 'file:///home/u/x', 'bookmark uppercase file');
assertEq(normalizeBookmarkUri('sftp://nas/My Documents'), 'sftp://nas/My%20Documents', 'bookmark sftp space');
...
assert(!bookmarkMatches('Code', '/home/u/Projects', 'ome'), 'ome is not a bookmark path');
...
assert(!resultRowShouldFocus(), 'rows must not steal entry focus');
assert(!popupChromeShouldFocus(), 'chrome must not steal entry focus');
const entry = {contains: actor => actor === 'clutter-text'};
assert(focusIsSearchEntry(entry, entry), 'entry is itself');
assert(focusIsSearchEntry('clutter-text', entry), 'entry owns clutter text');
assert(!focusIsSearchEntry('row', entry), 'a result row is not the entry');
assert(!focusIsSearchEntry(null, entry), 'null focus is not the entry');
assertEq(focusLossAction(false, false, false, false), 'refocus-entry', 'gnome 48 null focus returns to entry');
...
assert(shouldCaptureKeys(true, true, true, false), 'stage focus still captures escape');
...
assert(focusIsOnScreenKeyboard('osk-key', oskBox), 'key inside keyboard box');
assert(focusIsOnScreenKeyboard(oskBox, oskBox), 'keyboard box itself');
assert(!focusIsOnScreenKeyboard('other-window', oskBox), 'another actor is not osk');
assert(!focusIsOnScreenKeyboard(null, oskBox), 'null focus is not osk');
