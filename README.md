...
9. **Clock** — Type `time`, `now`, `what time is it`, `what's the time`, `date`, `today`, `tomorrow`, `yesterday`, or `clock` to copy the local time or date. Uses the session timezone via `GLib.DateTime`.
...
- Duration and area conversions (`2 hours to min`, `1 acre to m2`). Minutes are `min` so `m` stays meters.
