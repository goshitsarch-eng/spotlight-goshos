...
7. **Units** — Conversions such as `10 km to mi`, `1,000 km to mi`, `1e3 km to mi`, `.5 km to m`, `2 hours to min`, `1 acre to m2`, `32 f in c`, `32°f to c`, `1 stone to kg`, `1 nmi to km`, `1024 bytes to kib`, and `1 gb to mib`. Length, mass, temperature, US volume, SI/IEC data sizes, duration, and area. Press `Enter` to copy the converted value. Minutes use `min`, not `m` (meters).
