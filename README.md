...
1. **URLs** — `https://…`, `www.…`, a bare domain such as `example.com`, `host:port`, `localhost`, dotted IPv4, `[IPv6]`, or `*.local`. Also `sftp://`, `smb://`, `mailto:`, and `magnet:`. Local, LAN, mDNS, and IPv6 addresses open with `http`; public hosts use `https`. `javascript:` is rejected. Names that look like files (`node.js`, `readme.md`, `app.mjs`, `data.csv`) stay app and file searches.
